  ...
  <div class="min-h-screen bg-[#140F0A] p-4">
    <div class="max-w-7xl mx-auto space-y-8">
      ...
      <div class="text-center py-8">
        <h1 class="text-4xl md:text-5xl font-bold bg-gradient-to-r from-amber-400 to-yellow-500 bg-clip-text text-transparent mb-2">
          Săn Coupon Miễn Phí
        </h1>
        <p class="text-gray-300 text-lg">Nhận ngay các coupon hấp dẫn chỉ với một cú click!</p>
        <div class="mt-4 inline-flex items-center px-4 py-2 bg-gradient-to-r from-amber-600/20 to-yellow-600/20 rounded-full border border-amber-500/30">
          ...
          <span class="text-amber-300 text-sm font-medium">{{ collectedCount }}/{{ totalCoupons }} coupon đã thu thập</span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          <!-- Coupon Card -->
          <div class="bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl shadow-2xl overflow-hidden border border-amber-500/20 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-amber-500/20">
            ...
            <div v-if="coupon.isLimited" class="absolute top-4 left-4 z-10">
              <div class="px-3 py-1 bg-gradient-to-r from-orange-500 to-red-600 rounded-full text-white text-xs font-bold animate-pulse">
            ...
            <!-- Decorative Elements -->
            <div class="absolute -top-2 -right-2 w-8 h-8 bg-amber-500 rounded-full opacity-20 group-hover:opacity-40 transition-opacity"></div>
            <div class="absolute -bottom-2 -left-2 w-6 h-6 bg-yellow-500 rounded-full opacity-20 group-hover:opacity-40 transition-opacity"></div>
          </div>
        </div>
      </div>
      ...
      <Transition name="slide-up">
        <div v-if="showToast" class="fixed bottom-8 right-8 bg-gradient-to-r from-green-600 to-emerald-700 text-white px-6 py-4 rounded-2xl shadow-2xl border border-green-500/30 backdrop-blur-sm z-50">
